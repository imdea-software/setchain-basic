...
func init() {
	...
	flag.IntVar(&broadcastType, "broadcastType", 2, "Broadcast type: 0 = Immediate, 1 = PeriodicBroadcast, 2 = AfterEpoch")
	...
	flag.IntVar(&verificationTimeout, "verificationTimeout", 1000, "timeout for secondary verificators (milliseconds)")
...
func main() {
	...
	dpo := dpo.NewDPO(router, n, f, proposer, types.VerifySignedTransaction, types.BroadcastType(broadcastType), time.Duration(broadcastPeriod)*time.Millisecond, automaticEpochInc, time.Duration(verificationTimeout))
	...
	if nTX != 0 {
		...
		defer adderCancel()
		if !addToFPlus1 {
			go constantMempoolFiller(adderCtx, dpo, 1000*time.Millisecond, nTX)
		} else {
	...
	defer getterCancel()
	go periodicLengthGetterClient(getterCtx, dpo, 5)
...
// test function to fulfill mempool
func constantMempoolFiller(ctx context.Context, dpo *dpo.DPO, p time.Duration, q int) {
...
func simulateAddToFPlus1Servers(ctx context.Context, dpo *dpo.DPO, p time.Duration, q int, n int, f int) {
	ticker := time.NewTicker(p)
	_, sk, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		logger.Error.Printf("Error generating client keys: %v\n", err)
...
func periodicGetterClient(ctx context.Context, d *dpo.DPO, p time.Duration) {
...
// p must be in seconds
func periodicLengthGetterClient(ctx context.Context, d *dpo.DPO, p int) {
	var length int64
...
loop:
	for {
		select {
		default:
			length = d.GetCurrentLength()
			logger.Info.Printf("CurrentLenght: %d|%d/s\n", length, (length-previousLength)/int64(p))
...
func periodicEpochIncrement(ctx context.Context, dpo *dpo.DPO, p time.Duration) {
	var err error
loop:
	for {
		select {
...
func getFinalNumbers(dpo *dpo.DPO, fileName string) {
	f, err := os.OpenFile(fileName, os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
	...
	writer.Write([]string{
		...
		fmt.Sprint(dpo.GetAvgTimeToStamp()),
		fmt.Sprint(dpo.GetCurrentLength()),
...
func getPeriodicFinalNumbers(dpo *dpo.DPO, fileName string, p time.Duration) {
	for {
		time.Sleep(p)
